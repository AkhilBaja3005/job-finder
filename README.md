...
2. **Chrome Extension (`Job Finder ATS Tailor v2.0`)** — In-browser sidebar to score jobs, tailor resumes, auto-fill forms with multimodal intelligence, and dispatch delivery packages on LinkedIn, Indeed, Greenhouse, Lever, Ashby, Workday, and custom career sites.
...
## 🧩 Chrome Extension (`Job Finder ATS Tailor v2.0`)
...
- **Strict URL Scoping & Auto-Hide Cache**: Cache persists across popup auto-hides and automatically starts fresh when navigating to a new job posting.
...
- **Automatic Hot-Reloader (Local Dev)**: Automatically reloads the extension when files on disk change.
...
│   │   ├── resume_routes.py  # /parse_resume, /user/resume, /download_latex, /generate_html_resume
...
└── extension/            # Chrome Extension (Manifest V3)
    ├── manifest.json     # Extension permissions, host rules, and metadata (v2.0.0)
    ├── popup.html / js   # Popup interface for ATS scoring, tailoring, custom JD & email dispatch
    ├── content.js        # Universal job page text and metadata extractor & form autofiller
    └── background.js     # Service worker handling storage sync and hot-reloading
```
